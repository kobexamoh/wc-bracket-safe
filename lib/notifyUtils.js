...
/** Fingerprint group-stage picks only (ignores __knockout metadata). */
export function groupPicksFingerprint(rawPicks) {
  return JSON.stringify(validatePicks(rawPicks ?? {}));
}

function officialSubmittedAtFromPicks(rawPicks) {
  const picks = rawPicks && typeof rawPicks === 'object' ? rawPicks : {};
  const meta = picks.__knockout_real && typeof picks.__knockout_real === 'object' ? picks.__knockout_real : {};
  return typeof meta.submittedAt === 'string' ? meta.submittedAt : '';
}

function isNewOfficialSubmit(record, oldRecord) {
  const next = officialSubmittedAtFromPicks(record?.picks);
  if (!next) return false;
  const prev = officialSubmittedAtFromPicks(oldRecord?.picks);
  return next !== prev;
}

/**
 * Slack on first bracket row (INSERT) and when group-stage picks change (official
 * submit / resubmit). Skip UPDATEs that only touch knockout autosave (__knockout).
 */
export function shouldNotifyBracketWebhook(type, record, oldRecord) {
  if (type === 'INSERT') return true;
  if (type !== 'UPDATE') return false;
  if (!oldRecord || typeof oldRecord !== 'object') return true;
  if (groupPicksFingerprint(record?.picks) !== groupPicksFingerprint(oldRecord?.picks)) return true;
  return isNewOfficialSubmit(record, oldRecord);
}
...
export function parseSupabaseBracketEvent(body) {
  ...
  if (payload.table !== 'brackets') return null;
  ...
  const notify = shouldNotifyBracketWebhook(payload.type, row, payload.old_record);
  const action = payload.type === 'INSERT'
    ? 'submitted'
    : isNewOfficialSubmit(row, payload.old_record)
      ? 'official_submitted'
      : 'updated';
  return {
    action,
...
export function formatSlackBracket({ action, userId, updatedAt }) {
  ...
  const isOfficial = action === 'official_submitted';
  const verb = action === 'submitted' ? 'submitted' : 'updated';
  const headline = isOfficial ? 'official bracket submitted' : `bracket ${verb}`;
  const detail = isOfficial
    ? 'A signed-in user submitted their Official Bracket picks (champion through 4th place).'
    : `A signed-in user ${verb} their group-stage picks.`;
  return {
    text: `WC Bracket: ${headline}`,
    blocks: [
      {
        type: 'section',
        text: {
